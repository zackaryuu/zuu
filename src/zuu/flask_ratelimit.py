...
class FlaskRateLimit(SimpleRateLimit):
    ...
    def limit(self, method_name, max_requests=None, per=None, weight=1):
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                if self._is_rate_limited(client_ip, method_name, method_max_requests, window_seconds, request_weight):
                    if self.rate_limit_handler:
                        return self.rate_limit_handler(
                            method_name, 
                            method_max_requests, 
    ...
    def _calculate_weight(self, client_ip: str, method_name: str, base_weight: int) -> int:
        """
    ...
    def _unregister_connection(self, client_ip: str, method_name: str):
        ...
        for key, conn in self._active_connections.items():
            if (conn['client_ip'] == client_ip and 
                conn['method_name'] == method_name and 
                conn['start_time'] > latest_time):
                to_remove = key
    ...
    def _is_rate_limited(self, client_ip: str, method_name: str, max_requests: int, window_seconds: int, weight: int) -> bool:
        """
        ...
        request_data[:] = [req for req in request_data if req.get('timestamp', req) > cutoff_time]
    ...
    def get_concurrent_stats(self) -> dict:
        ...
        current_weight = sum(conn['weight'] for conn in self._active_connections.values())
        
        return {
            'active_connections': len(self._active_connections),
            'current_weight': current_weight,
            'max_allowed_concurrent': self.max_allowed_concurrent,
            'weight_utilization_percent': (current_weight / self.max_allowed_concurrent) * 100,
    ...
    def evict_lru(self, count: int = 1):
        ...
        for _ in range(min(count, len(self._lru_access))):
            if self._lru_access:
                ...
                if (method_name in self._internal_tracking and 
                    client_ip in self._internal_tracking[method_name]):
